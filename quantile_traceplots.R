...
quantile_traceplots_dataset <- function(dataset, datasets, block_size=100) {
  quant_alpha = calculate_quantiles(datasets, "alpha.tsv")
  quant_K = calculate_quantiles(datasets, "K.txt")
  quant_latent = calculate_quantiles(datasets, "pLatentsGivenClusters.tsv")
  ...
  g = grid.arrange(g_alpha + theme(legend.position = "none"),
                   ...
                   ncol=4,
                   widths=c(2,2,2,1))
  ggsave(paste0("./plots/trace_median_", dataset, ".png"), g, width=12, height=7, units="in")
...
burren_traceplots <- function(directory) {
  ...
  dir.create(paste0(directory, "/plots/"), recursive = TRUE)
  datasets = paste0(directory, "/trimmed_results/with_subtypes_001_novar/seed", 1001:1010, "/")
  quantile_traceplots_dataset("with_subtypes_001_novar", datasets)
  dir.create(paste0(directory, "/plots/"), recursive = TRUE)
  datasets = paste0(directory, "/trimmed_results/with_subtypes_noGA_001_novar/seed", 1001:1010, "/")
  quantile_traceplots_dataset("with_subtypes_noGA_001_novar", datasets)
}
