...
compare_versions <- function(psm_results, base_run, plots_dir) {
    ...
    all_calls_mclust = all_calls_mclust %>% data.frame() %>% select(!ends_with("novar"))
    ...
    all_calls_kmeans = all_calls_kmeans %>% data.frame() %>% select(!ends_with("novar"))
    print(all_calls_kmeans)

    colnames(all_calls) = paste0("DPMUnc__", colnames(all_calls))
    # exclude kmeans since K is so high!
    all_calls = cbind(all_calls, all_calls_mclust)#, all_calls_kmeans)
    print(apply(all_calls, MARGIN=1, unique))

    colnames(all_calls) = str_replace(colnames(all_calls), "__with_subtypes_noblood_noGA_001", "")
...
#                                   grid.text(cell_labels[i, j], x, y, gp = gpar(fontsize = 10))
#                       },
    ...
    png(paste0(plots_dir, "/combined_heatmap.png"), width=7, height=10, units="in", res=1200)
    draw(ch_calls + ch_obs + ch_vars, merge_legend=TRUE, heatmap_legend_side = "bottom")
    dev.off()
