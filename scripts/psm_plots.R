...
psm_plots <- function(dataset, datasets, name, focus_dataset=NULL) {
    obsData = read.table(paste0(dataset, "/beta.tsv"),
                         header=1, row.names=1)
    obsVars = read.table(paste0(dataset, "/var.tsv"),
                         header=1, row.names=1)
    ...
    obs_vars_heatmap = pheatmap(log10(obsVars),
                                cluster_rows = hclust.comp,
                                cluster_cols = FALSE,
                                annotation_colors = annotations$colors,
                                annotation_row = annotations$ann,
                                ...
                                width=9,
                                height = 14,
                                filename=paste0("plots/", name, "/obs_vars_log_heatmap.png"))
